...
func TestNrNrqlAlerConditionInvalidAggregationDelayRule(t *testing.T) {
	tests := []struct {
		Name     string
		Content  string
...
resource "newrelic_nrql_alert_condition" "monitor" {
  ...
  aggregation_delay = 60
			...
			Expected: helper.Issues{
				{
					Rule:    NewNrNrqlAlerConditionInvalidAggregationDelayRule(),
...
resource "newrelic_nrql_alert_condition" "monitor" {
  ...
  aggregation_delay = 1500
			...
			Expected: helper.Issues{
				{
					Rule:    NewNrNrqlAlerConditionInvalidAggregationDelayRule(),
					Message: "'1500' invalid value for aggregation_delay with aggregation_method 'event_flow'",
					Range: hcl.Range{
						Filename: "resource.tf",
						Start:    hcl.Pos{Line: 4, Column: 24},
...
resource "newrelic_nrql_alert_condition" "monitor" {
  ...
  aggregation_delay = 3900
			...
			Expected: helper.Issues{
				{
					Rule:    NewNrNrqlAlerConditionInvalidAggregationDelayRule(),
					Message: "'3900' invalid value for aggregation_delay with aggregation_method 'cadence'",
					Range: hcl.Range{
						Filename: "resource.tf",
						Start:    hcl.Pos{Line: 4, Column: 24},
